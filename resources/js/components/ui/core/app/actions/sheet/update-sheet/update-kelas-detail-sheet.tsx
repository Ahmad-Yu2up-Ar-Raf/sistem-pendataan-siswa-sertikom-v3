// src/components/sheets/UpdateKelasDetailSheet.tsx
"use client";
...
interface Props {
  kelasDetail: KelasDetailSchema | null;
  open?: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function UpdateKelasDetailSheet({ kelasDetail, open = false, onOpenChange }: Props) {
  ...
  const route = kelasDetail?.id ? `/dashboard/kelasDetail/${kelasDetail.id}` : "/dashboard/kelasDetail";

  const { form, submit, isPending } = useKelasDetailForm(kelasDetail ?? undefined, {
    notify: ({ type, message }) => {
    ...
    route,
    method: kelasDetail?.id ? "put" : "post", // if no id treat as create fallback
  });
    ...
    return (
      <Sheet open={internalOpen} onOpenChange={setInternalOpen} modal={true}>
        ...
        <SheetContent className="flex flex-col gap-6 overflow-y-scroll ">
          ...
          <KelasDetailForm<KelasDetailSchema>
            isPending={isPending}
            form={form}
            onSubmit={submit}
          >
